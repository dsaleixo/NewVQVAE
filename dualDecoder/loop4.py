...
from dualDecoder.dualDecoder import DualDecoder
from setup_env import PROJECT_ROOT
from oh_new_decoder.modelDec import ModelGridTemporalVQVAE
...
def initialProcess(model,valLoader,device):
        ...
        x_rec0,x_rec1, vq_loss, indices, perplexity, used_codes = model(x)   
        x_rec = x_rec0.squeeze()  
        ...
        Viewer.saveListTensorAsImg(imgs,f"RecImagemVal0_{i}",f"match{i}")
        Viewer.saveTensorAsGIF(imgs,f"RecVideoVal0_{i}",f"match{i}")

        x_rec = x_rec1.squeeze()  
        pt = visu0(x_rec,labels)
        pt0 = visu1(x_rec,labels)
        pt1 = visu2(x_rec,labels)
        pt2 = visu3(x_rec,labels)
        ...
        imgs = [x.squeeze(),x_rec,pt2,pt1,pt0,pt]
        Viewer.saveListTensorAsImg(imgs,f"RecImagemVal1{i}",f"match{i}")
        Viewer.saveTensorAsGIF(imgs,f"RecVideoVal1{i}",f"match{i}")
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(trainLoader):
            ...
            logits0, logits1, vq_loss, indices, perplexity, used_codes = model(x,epoch>epochVQturnOn)  
            ...
            wandb.log({
                    ...
                    "Train/Recon Loss0": recon_loss0.item(),
                    "Train/Recon Loss_focal0": loss_focal0.item(),
                    "Train/Recon Loss1": recon_loss1.item(),
                    "Train/Recon Loss_focal1": loss_focal1.item(),
            ...
            print(
                ...
                f"Loss: {loss.item():.4f}, Recon: {recon_loss0.item():.4f}, "
                f" Loss_focal: {loss_focal0.item():.4f}, "
